...
predict_cluster_by_category_osm <- function (k, probs){
  ...
  cluster_space <- 1:length(probs)
  
  cluster_probabilities <- sapply(probs, function(x) x[k])
  
  # Normalize the cluster probabilities
  cluster_probabilities <- cluster_probabilities / sum(cluster_probabilities)
  
  # Generate 1 observation as predict based on the provided cluster space and probabilities
  predict_cluster <- sample(cluster_space, size = 1, replace = TRUE, prob = cluster_probabilities)
