...
number_of_y = 10
alpha=c(1,-1) 
beta=c(-0.0001, -0.02, 0, 0.01, 0.02, 0.03, 0.05, 0.02, 0.9, 1.2)
mu=c(0, 0.6, 0.3) 
phi=c(0, 0.8, 1)
...
y_sampling <- function(sample_size, total_sample_size, cluster_pi, q, beta, y_idx) {
  ## prob matrix
  ...
  for (g in 1:G) {
    probs <- numeric(q)
    for (k in 1:q) {
      linear <- mu[k] + phi[k] * (alpha[g] + beta[y_idx])
      if (k > 1) {
        probs[k] <- exp(linear)
      } else {
        probs[k] <- 1
...
data_samping <- function(sample_size, total_sample_size, cluster_pi, q, 
  ...
  dataframes <- lapply(1:number_of_y, function(i) y_sampling(sample_size, total_sample_size, cluster_pi, q, beta, i))
