...
training <- function(df, number_of_y){
  ...
  results <- clustord(Y~ROWCLUST+COL,model="OSM",8,long.df=df, EM.control=list(EMcycles=100,startEMcycles=2), nstarts=20)
...
evaluation <- function(predicted, actual){
  ...
  predicted <- factor(predicted, levels = c(1, 2, 3,4,5,6,7,8))
  actual <- factor(actual, levels = c(1, 2, 3,4,5,6,7,8))
