...
alpha <- c(5.073905, 3.690511, -8.764416)
alpha_exp <- exp(alpha)
alpha_normalized <- alpha_exp / sum(alpha_exp)
...
# Using the log-sum-exp trick for normalization
max_alpha <- max(alpha)
alpha_adj <- alpha - max_alpha  # Subtract the maximum alpha value for stability
alpha_exp <- exp(alpha_adj)
alpha_normalized <- alpha_exp / sum(alpha_exp)
